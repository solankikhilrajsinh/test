...
if ($post_data) 
{
    ...
    if (isset($_FILES['profile_image']) && $_FILES['profile_image']['error'] == 0) {
        ...
        if (move_uploaded_file($_FILES['profile_image']['tmp_name'], $target_file)) {
            ...
            $post_data['profile_image'] = $file_name;
        } else {
    ...
    if (!$error) {
        //WE ARE INCLUDING FILE DIRECTLY
        ...
        $error = $db->check_phone($post_data['phone']);
        if($error){
        ...
        $error = $db->check_password($post_data['password']);
        if($error){
        ...
        if (!$error) {
            $result = $db->add_user($post_data);
            if($result == 'Success'){
                $message = "Added Successfully";
                    header("Location: list_pagination.php");
                    exit();
            }else{
                $message = $result;
            }
